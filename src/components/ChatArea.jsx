...
const ChatArea = ({ data }) => {
  ...
  const { handleToggleSidebar, isSidebarVisible, setIsSidebarVisible } = data;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        setMessages((prevMessages) => [
          ...prevMessages,
          { text: agroResponse, isUser: false, show: false },
        ]);
  ...
  return (
    <>
      <div className="chat w-full flex px-6 flex-col pt-20 max-[640px]:ml-0 justify-between items-center">
        {messages.length === 0 ? (
          <>
            ...
            <div className="chat-topic text-sm flex flex-col gap-8 items-center mt-10">
              <div className="flex items-center gap-6">
                <p className="py-2 px-6 rounded-lg text-center text-xs">
                  Soil Management
                </p>
                <p className="py-2 px-6 rounded-lg text-center text-xs">
                  Nutrient Management
              ...
              >
                <div className="flex gap-1 items-start">
                  {message.isUser ? (
                    <>
                      <p
                        style={{ fontSize: ".65rem", padding: ".1rem .5rem" }}
                        className="bg-slate-50 w-max rounded-full text-green-900 font-normal"
                      >
                        {user.first_name}
                      </p>
                      <p>{message.text}</p>
                ...
                <small
                  className={`mt-1 text-slate-300 ${
                    message.isUser ? "self-start text-slate-200" : ""
                  }`}
                >
                  {timestamp}
                </small>
        ...
        <form
          ...
          className="w-full flex message-input py-4 px-4 rounded-lg items-center gap-4 mb-4 fixed bottom-0 w-80 sm:w-1/2"
        >
      ...
      <button
        className={`bg-transparent flex flex-col items-center justify-center h-fit m-auto py-2 px-2 hide-chat-history side-btn `}
        onClick={handleToggleSidebar}
        title="Hide chat history"
        style={isSidebarVisible ? { left: "" } : { left: "" }}
      >
        <div
          className={`h-4 -m-1 w-1 rounded-full bg-green-600 rot-up`}
          style={isSidebarVisible ? { rotate: "" } : { rotate: "-35deg" }}
        ></div>
        <div
          className={`h-4 w-1 rounded-full bg-green-600 rot-down`}
          style={isSidebarVisible ? { rotate: "" } : { rotate: "35deg" }}
        ></div>
      </button>
    </>
